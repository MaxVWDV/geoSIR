...
% Random number generator state. Shuffle = true randomness, integer =
% repeatable randomness.
rng('shuffle');


inputs.Name = 'V1_1';      %Name to save this run parameters under

inputs.Number_of_people = 5800000;                                           %Population of area
...
inputs.Hospital_capacity = 427;                                              %Hospotial capacity for seriously ill patients (if exceeded, death rate rises)
...
inputs.Is_a_lockdown_implemented = 'No';                                   %Is a lockdown implemented if certain conditions are met?

inputs.Confirmed_cases_lockdown_threshold = 2000;                            %Number of confirmed cases needed before a lockdown is implemented
...
inputs.Deaths_lockdown_exit = -1;                                           %Maximum number of daily deaths needed before a lockdown lifted
    %Lockdown not lifted
inputs.Lockdown_proportion_non_symptomatic.min = 0.85;                       %Efficiency of lockdown for non-symptomatic people
inputs.Lockdown_proportion_non_symptomatic.max = 0.85;

inputs.Lockdown_proportion_symptomatic.min = 0.975;                           %Efficiency of lockdown for symptomatic people
inputs.Lockdown_proportion_symptomatic.max = 0.975;
...
inputs.Confirmed_cases_travel_ban_threshold = 500;                          %Number of confirmed cases needed before a lockdown is implemented

inputs.Deaths_travel_ban_threshold = 50;                                    %Number of deaths needed before a lockdown is implemented.
...
inputs.Maximum_time = 180;                                                 %Maximum number of days that the model runs for

inputs.Number_of_iterations = 1;                                           %Number of individual iterations run

inputs.Parralelize = 'No';                                                 %Run in parralel? Will default to maximum number of cores of user.

inputs.Number_of_iterations_for_R = 10;                                     %Number of iterations for calculating the reproductive number and 5 day progression

inputs.Is_there_a_ND = 'No';                                        %Does an ND happen?

inputs.ND_case = 3;                                                   %ND case. See run file for details.
...
inputs.maximum_density = 1;
                                                                            ...
                                                                            %Paths to population and hazard files on computer

inputs.path_to_population = '/panfs/roc/groups/14/awickert/vanwy048/Documents/models/geoSIR_model/Italy population/population_amount.tif';

inputs.path_to_red = '/panfs/roc/groups/14/awickert/vanwy048/Documents/models/geoSIR_model/Italy population/Red_zone.jpg';

inputs.path_to_yellow = '/panfs/roc/groups/14/awickert/vanwy048/Documents/models/geoSIR_model/Italy population/Yellow_zone.jpg';
